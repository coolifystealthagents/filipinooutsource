...
const batchPath='app/blog-september24.ts', sharedPath='app/blog-september23-shared.ts', manifestPath='.paperclip/daily-content/2026-09-24/blog.json';
...
const other=fs.readdirSync('app').filter(n=>n.startsWith('blog-')&&n.endsWith('.ts')&&!['blog-september24.ts'].includes(n)).map(n=>fs.readFileSync(`app/${n}`,'utf8')).join('\n');
...
if(!data.includes('...september24BlogPosts')||!data.includes('...september24BlogDetails'))throw new Error('Batch not wired into data.ts');
...
if(manifest.requiredCount!==12||manifest.entries.length!==12||manifest.publicationDate!=='2026-09-24')throw new Error('Manifest count/date mismatch');
...
console.log(`SEPTEMBER24_BLOG_PASS count=${slugs.length} bodyWords=${bodyWords} manifest=${manifestPath}`);
