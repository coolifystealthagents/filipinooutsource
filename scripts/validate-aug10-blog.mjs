...
for (const entry of manifest.entries) {
  ...
  if (entry.provenance === 'original-aug10-batch') {
    ...
    if (before.includes('aug10PublicSourceDates') || !after.includes(`'${entry.slug}': '2026-08-10'`)) throw new Error(`diff provenance failed ${entry.slug}`);
    if (entry.sourceDateField !== `aug10PublicSourceDates.${entry.slug}`) throw new Error(`bad repair date field ${entry.slug}`);
  }
  if (entry.sourceDate !== '2026-08-10' || entry.renderedDate !== '2026-08-10') throw new Error(`bad date ${entry.slug}`);
  if (!source.includes(`'${entry.slug}': '2026-08-10'`) || !source.includes('datePublished: aug10PublicSourceDates[slug]')) throw new Error(`slug/date absent in source ${entry.slug}`);
