...
export default async function Post({params}:{params:Promise<{slug:string}>}){
  ...
  if(detail){
    ...
    const articleSchema={'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:detail.datePublished||'2026-07-22',dateModified:detail.dateModified||'2026-07-22',author:{'@type':'Organization',name:`${site.brand} Editorial Team`},publisher:{'@type':'Organization',name:site.brand,url:base},image:detail.image?`${base}${detail.image.src}`:undefined,citation:detail.sources?.map((source:any)=>source.url),mainEntityOfPage:url};
